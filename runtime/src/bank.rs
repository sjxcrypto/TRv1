...
impl Bank {
    ...
    fn process_new_epoch(
        ...
        // TRv1: At epoch boundary, check for offline validators and auto-jail
        // them if they haven't voted in the last `offline_jail_threshold` slots.
        {
            let stakes = self.stakes_cache.stakes();
            let vote_accounts = stakes.vote_accounts();
            let mut last_vote_slots = std::collections::HashMap::new();
            for (_vote_pubkey, vote_account) in vote_accounts.iter() {
                let vote_state = vote_account.vote_state_view();
                let node_pubkey = *vote_state.node_pubkey();
                let last_vote = vote_state.last_voted_slot().unwrap_or(0);
                // Track the latest vote slot across all vote accounts for each node.
                let entry = last_vote_slots.entry(node_pubkey).or_insert(0u64);
                if last_vote > *entry {
                    *entry = last_vote;
                }
            }
            drop(stakes);
            let newly_jailed = self
                .trv1_slashing
                .write()
                .unwrap()
                .check_offline_validators(&last_vote_slots, slot, epoch);
            if !newly_jailed.is_empty() {
                info!(
                    "TRv1: auto-jailed {} offline validators at epoch {}: {:?}",
                    newly_jailed.len(),
                    epoch,
                    newly_jailed
                );
    ...
    /// TRv1: read-only access to slashing/jailing state.
    pub fn trv1_slashing_state(&self) -> std::sync::RwLockReadGuard<'_, crate::slashing::SlashingState> {
        self.trv1_slashing.read().unwrap()
    }
    ...
    /// TRv1 inflation model: flat 5% annual staking rate applied to staked supply only.
    ///
    /// Unlike Solana's declining inflation on total supply, TRv1 mints new tokens
    /// proportional to the staked amount: `new_tokens = staked_supply * STAKING_RATE / epochs_per_year`.
    /// This means actual inflation rate depends on staking participation.
    fn calculate_epoch_inflation_rewards(
        &self,
        _capitalization: u64,
        epoch: Epoch,
    ) -> EpochInflationRewards {
        use crate::trv1_constants::STAKING_RATE;

        // Get total staked supply from the stake history for the previous epoch
        ...
        // TRv1: rewards = staked_supply * 5% * (epoch_duration / year)
        let validator_rewards_lamports =
            (staked_supply as f64 * STAKING_RATE * epoch_duration_in_years) as u64;

        // validator_rate represents the effective inflation rate relative to total supply
        // foundation_rate is 0 in TRv1 (no foundation allocation from inflation)
        let validator_rate = STAKING_RATE;
        let foundation_rate = 0.0_f64;
