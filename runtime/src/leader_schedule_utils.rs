use {
    crate::{bank::Bank, trv1_active_set::ActiveValidatorSet},
...
///
/// **TRv1 change**: only the top [`MAX_ACTIVE_VALIDATORS`] validators
/// (by total stake) are eligible for leader slots.  Jailed/banned
/// validators are excluded.  Standby validators never appear in the
/// leader schedule.
pub fn leader_schedule(epoch: Epoch, bank: &Bank) -> Option<LeaderSchedule> {
    bank.epoch_vote_accounts(epoch).map(|vote_accounts_map| {
        // Compute which validators are in the active set.
        let jailed = bank.trv1_slashing_state().jailed_set();
        let active_set =
            ActiveValidatorSet::compute(vote_accounts_map, &jailed);

        // Filter vote accounts to only include those whose node identity
        // is in the active set.
        let active_vote_accounts =
            active_set.filter_active_vote_accounts(vote_accounts_map);

        LeaderSchedule::new(
            &active_vote_accounts,
            epoch,
