...
impl Bank {
    ...
    fn redeem_delegation_rewards(
        ...
        // TRv1: Skip staking rewards for delegations to jailed/banned validators.
        // Jailed and permanently-banned validators earn 0 staking rewards.
        // Their delegators also receive 0 for the jailed epoch — this
        // incentivises delegators to redelegate away from bad validators.
        {
            let node_pubkey = vote_state.node_pubkey();
            let slashing = self.trv1_slashing.read().unwrap();
            if slashing.is_jailed_or_banned(node_pubkey) {
                return None;
            }
        }
