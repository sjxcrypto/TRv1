use {
    crate::{
        address_lookup_table::*, clap_app::*, cluster_query::*, developer_rewards::*, feature::*,
        governance::*, inflation::*, network_info::*, nonce::*, passive_stake::*, program::*,
        program_v4::*, spend_utils::*, stake::*, treasury::*, trv1_validators::*,
...
pub enum CliCommand {
    ...
    // TRv1-specific commands
    PassiveStake(PassiveStakeCliCommand),
    Treasury(TreasuryCliCommand),
    Governance(GovernanceCliCommand),
    DevRewards(DevRewardsCliCommand),
    Trv1Validators(Trv1ValidatorsCliCommand),
    NetworkInfo(NetworkInfoCliCommand),
}
...
pub fn parse_command(
    ...
    let response = match matches.subcommand() {
        ...
        // TRv1 Commands
        ("passive-stake", Some(matches)) => {
            parse_passive_stake_command(matches, default_signer, wallet_manager)
        }
        ("treasury", Some(matches)) => {
            parse_treasury_command(matches, default_signer, wallet_manager)
        }
        ("governance", Some(matches)) => {
            parse_governance_command(matches, default_signer, wallet_manager)
        }
        ("dev-rewards", Some(matches)) => {
            parse_dev_rewards_command(matches, default_signer, wallet_manager)
        }
        ("trv1-validators", Some(matches)) => {
            parse_trv1_validators_command(matches, default_signer, wallet_manager)
        }
        ("network", Some(matches)) => {
            parse_network_info_command(matches, default_signer, wallet_manager)
        }
        //
        ("", None) => {
...
pub async fn process_command(config: &CliConfig<'_>) -> ProcessResult {
    ...
    match &config.command {
        ...
        // TRv1-specific commands
        CliCommand::PassiveStake(subcommand) => {
            process_passive_stake_command(&rpc_client, config, subcommand).await
        }
        CliCommand::Treasury(subcommand) => {
            process_treasury_command(&rpc_client, config, subcommand).await
        }
        CliCommand::Governance(subcommand) => {
            process_governance_command(&rpc_client, config, subcommand).await
        }
        CliCommand::DevRewards(subcommand) => {
            process_dev_rewards_command(&rpc_client, config, subcommand).await
        }
        CliCommand::Trv1Validators(subcommand) => {
            process_trv1_validators_command(&rpc_client, config, subcommand).await
        }
        CliCommand::NetworkInfo(subcommand) => {
            process_network_info_command(&rpc_client, config, subcommand).await
        }
    }
}
